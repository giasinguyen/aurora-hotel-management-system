...
public class DashboardServiceImpl implements DashboardService {
    ...
    public List<CustomerGrowthPoint> getCustomerGrowth(DashboardGroupBy period) {
        ...
        List<com.aurora.backend.entity.User> customers = userRepository.findAllByRoleName("CUSTOMER");
        ...
        for (var customer : customers) {
            ...
            LocalDate createdDate = customer.getCreatedAt().toLocalDate();
            LocalDate bucketKey = bucket.normalize(createdDate);
            aggregated.merge(bucketKey, 1L, Long::sum);
        }

        return aggregated.entrySet().stream()
                .map(entry -> CustomerGrowthPoint.builder()
                        .periodLabel(formatPeriodLabel(bucket, entry.getKey()))
    ...
    private DashboardOverviewResponse buildOverview(DateRange range, String branchId) {
        ...
        double occupancyRate = calculateOccupancyRate(range.end(), branchId);
    ...
    private double calculateOccupancyRate(LocalDate date, String branchId) {
        ...
        return (occupiedRooms * 100d) / totalRooms;
    }
